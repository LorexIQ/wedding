...
`CONTACT_PHONE` (footer contact number, digits only, e.g. `79001234567`; falls back to the placeholder in `app/content/wedding.ts`'s `contactDigits` if unset). Unlike `METRIKA_ID`, this one deliberately goes through Nuxt's automatic `NUXT_PUBLIC_`-prefixed runtime-config override (`NUXT_PUBLIC_CONTACT_PHONE` in Docker — see `docker-compose.yml`), not a raw `process.env` read in `nuxt.config.ts`. That matters because Nitro re-reads `NUXT_PUBLIC_*` env vars at container **startup**, whereas a raw `process.env.CONTACT_PHONE` read in `nuxt.config.ts` would only see whatever was set during the Docker **build** step (`RUN npm run build`) — `docker-compose.yml`'s `environment:` never reaches that build stage, so the value would silently bake in as the fallback and never update without a full image rebuild. `NUXT_PUBLIC_CONTACT_PHONE` lets the number change with just a container restart (`docker compose up -d`, no `--build`).
